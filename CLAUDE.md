...
- `.vscode/settings.json` is the canonical VS Code user settings; `run_onchange_vscode-settings.sh.tmpl` symlinks it into every installed VS Code-family editor's `User/` dir (Code, Code - Insiders, VSCodium, Cursor, Antigravity[+ IDE]) — base is `~/Library/Application Support` on macOS, `~/.config` on Linux/Codespaces. Keep this file free of machine-specific paths (the secret-scan hook blocks `/Users/<name>`)
