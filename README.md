...
| **VS Code settings** | one canonical `settings.json` symlinked into every installed VS Code-family editor | macOS, Linux, Codespaces |
...
│
├── .vscode/settings.json            Canonical VS Code user settings (source of truth for all editors)
├── run_onchange_vscode-settings.sh.tmpl  Symlinks the above into every installed VS Code-family editor
│
...
`.vscode/settings.json` is the single source of truth for **user-level** VS Code settings.
`run_onchange_vscode-settings.sh.tmpl` symlinks it into the `User/` config dir of every installed
VS Code-family editor, so they all share one file — edit it once, every editor updates live:

| Editor | macOS dir | Linux / Codespaces dir |
|---|---|---|
| VS Code | `~/Library/Application Support/Code/User/` | `~/.config/Code/User/` |
| VS Code Insiders | `Code - Insiders` | `Code - Insiders` |
| VSCodium | `VSCodium` | `VSCodium` |
| Cursor | `Cursor` | `Cursor` |
| Antigravity | `Antigravity` and `Antigravity IDE` | same |

Only **installed** editors (config dir present) are linked; the rest are skipped. A pre-existing
real `settings.json` is moved to `settings.json.pre-symlink.bak` before linking — never destroyed.
To pick up a newly installed editor, run `chezmoi apply --force`. The canonical file is kept free
of machine-specific values (absolute `/Users` paths, per-machine tool paths) so it's portable.
